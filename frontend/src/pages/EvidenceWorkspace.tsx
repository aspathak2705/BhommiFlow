...
interface InterventionCandidate {
  ...
  priority: string;
  expected_process_effect: string;
...
export default function EvidenceWorkspace() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch Phase 5 Explanation
      const expRes = await fetch(`/api/v1/projects/${projectId}/explanation`, { headers });
      if (expRes.ok) {
        const expData = await expRes.json();
        setExplanation(expData);
      }

      // Fetch Phase 5 Interventions
      const intRes = await fetch(`/api/v1/projects/${projectId}/interventions`, { headers });
      if (intRes.ok) {
        ...
        <>
          ...
          {activeTab === 'intelligence' && (
            <div className="space-y-6">
                ...
                <div className="bg-slate-800/60 rounded-xl p-5 border border-emerald-500/30">
                  ...
                  <div className="bg-slate-900 p-4 rounded-lg border border-slate-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      {prediction.top_contributing_features.map((feat: { feature: string; value: number; importance: number }, idx: number) => (
              ...
              {/* Phase 5: Why This Project Is At Risk (Explainability Card) */}
              {explanation && (
                <div className="bg-slate-800/60 rounded-xl p-5 border border-indigo-500/30">
                  <h2 className="text-sm font-semibold text-white mb-2 flex items-center gap-2">
                    <AlertTriangle className="w-4 h-4 text-indigo-400" />
                    Why This Project Is At Risk (Explainability Intelligence)
                  </h2>
                  <p className="text-xs text-slate-300 mb-4 bg-slate-900/60 p-3 rounded border border-slate-700">
                    {explanation.summary}
                  </p>

                  <div className="space-y-4">
                    <div>
                      <div className="text-xs font-semibold text-slate-300 mb-2">Linked Evidence Drivers:</div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {explanation.top_drivers.map((drv, idx) => (
                          <div key={idx} className="bg-slate-900 p-3 rounded-lg border border-slate-700">
                            <div className="flex items-center justify-between">
                              <span className="text-xs font-bold text-indigo-300">{drv.human_label}</span>
                              <span className="text-[10px] bg-slate-800 text-slate-400 px-2 py-0.5 rounded font-mono">
                                Impact: +{(drv.importance * 100).toFixed(1)}%
                              </span>
                            </div>
                            <div className="text-xs text-slate-400 mt-1">Observed Value: <span className="text-white font-mono">{drv.observed_value}</span></div>
                            {drv.supporting_evidence_type && (
                              <div className="text-[11px] text-emerald-400 mt-2 font-mono flex items-center gap-1">
                                <CheckCircle2 className="w-3 h-3 text-emerald-400 inline" /> Linked Evidence: {drv.supporting_evidence_type} ({drv.supporting_source_id || 'Extracted'})
                              </div>
                            )}
                          </div>
                        ))}
                    ...
                    {explanation.uncertainties && explanation.uncertainties.length > 0 && (
                      <div>
                        <div className="text-xs font-semibold text-amber-300 mb-2">Uncertainty & Data Completeness Disclaimers:</div>
                        <div className="space-y-2">
                          {explanation.uncertainties.map((u, idx) => (
                            <div key={idx} className="bg-amber-500/10 border border-amber-500/20 p-2.5 rounded text-xs">
                              <span className="font-bold text-amber-300">{u.title}: </span>
              ...
              {/* Phase 5: Recommended Officer Interventions Card */}
              {interventions && interventions.length > 0 && (
                <div className="bg-slate-800/60 rounded-xl p-5 border border-amber-500/30">
                  <h2 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-amber-400" />
                    Recommended Officer Interventions (Actionable Decision Support)
                  </h2>
                  <div className="space-y-3">
                    {interventions.map((intv) => (
                      <div key={intv.intervention_id} className="bg-slate-900 p-4 rounded-lg border border-slate-700">
                        <div className="flex items-center justify-between">
                          <h3 className="text-xs font-bold text-amber-300">{intv.title}</h3>
                          <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${
                            intv.priority === 'HIGH' ? 'bg-red-500/20 text-red-400 border border-red-500/30' :
                            intv.priority === 'MEDIUM' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' :
                            'bg-slate-800 text-slate-400 border border-slate-700'
                          }`}>
                            {intv.priority} Priority
                          </span>
                        </div>
                        <p className="text-xs text-slate-300 mt-1">{intv.description}</p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-3 pt-3 border-t border-slate-800 text-xs">
                          <div>
                            <span className="text-slate-500">Target Bottleneck: </span>
                            <span className="text-slate-300 font-medium">{intv.target_bottleneck}</span>
                          </div>
                          <div>
                            <span className="text-slate-500">Expected Process Effect: </span>
                            <span className="text-emerald-400 font-medium">{intv.expected_process_effect}</span>
                          </div>
                        </div>
                        {intv.required_evidence && (
                          <div className="text-[11px] text-slate-400 mt-2 bg-slate-800/80 p-2 rounded border border-slate-700 font-mono">
                            Required Verification Evidence: {intv.required_evidence}
                          </div>
                        )}
                      </div>
                    ))}
